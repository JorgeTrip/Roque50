...
window.inicializarCuentaRegresiva = function() {
  ...
  const actualizar = () => {
    ...
    if (diff <= 0) {
      el.innerHTML = '<div class="cd-dias">¡YA ES HOY! 🎉</div>';
      return;
    }
    ...
    el.innerHTML = `
      <div class="cd-dias">${dias} DÍAS</div>
      <div class="cd-tiempo">${formatear(horas)} horas · ${formatear(min)} minutos · ${formatear(seg)} segundos</div>
    `;
  };
